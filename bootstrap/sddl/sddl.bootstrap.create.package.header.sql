...
/* SPs */
SET TERM ^ ;
CREATE OR ALTER PROCEDURE SP_SDDL_GET_CURRENT_USER
RETURNS (
...
COMMENT ON PROCEDURE SP_SDDL_GET_CURRENT_USER
IS 'Wrapper SP für den CURRENT_USER'^

CREATE OR ALTER PROCEDURE SP_SDDL_SHOW_INFO(
  AInfo DN_MESSAGE)
RETURNS (
  INFO DN_MESSAGE)
AS
...
CREATE OR ALTER PROCEDURE SP_SDDL_RESTRUCTURE_TABLE (
...
CREATE OR ALTER PROCEDURE SP_SDDL_EXTRACT_TABLENAME (
  ATablename DN_DB_OBJECT,
  APrefix DN_PREFIX)
RETURNS (
...
COMMENT ON PROCEDURE SP_SDDL_EXTRACT_TABLENAME IS
'Entfernt den Prefix aus einem Tabellennamen'^

CREATE OR ALTER PROCEDURE SP_SDDL_GET_COLUMNLIST(
  ATablename DN_DB_OBJECT,
...
COMMENT ON PROCEDURE SP_SDDL_GET_COLUMNLIST IS
'Ermittelt die Spaltenliste zu einem Tabellenamen'^

CREATE OR ALTER PROCEDURE SP_SDDL_GET_PRIMKEYLIST (
  ATablename DN_DB_OBJECT,
  ASeparator DN_COLUMNLIST_SEPARATOR,
  ADoCR DN_BOOLEAN)
returns (
...
COMMENT ON PROCEDURE SP_SDDL_GET_PRIMKEYLIST IS
'Ermittelt eine Spaltenliste der Primärschlüssel'^

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_STD_TABLE_VIEW_CMT (
  ATablename DN_DB_OBJECT,
...
COMMENT ON PROCEDURE SP_SDDL_GEN_STD_TABLE_VIEW_CMT 
IS 'Erstellt anhand der Tabellenkommentare, die Standardviewkommentare'^

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_STD_TABLE_VIEW(
  ATablename DN_DB_OBJECT,
...
begin
  ...
  from SP_SDDL_EXTRACT_TABLENAME(:ATablename, 'TB')
  ...
  from SP_SDDL_GET_COLUMNLIST(:ATablename, ',', True) 
  into :columnlist;
  ...
  begin 
    ...
    from SP_SDDL_GET_PRIMKEYLIST(:ATablename, ',', True) 
  ...
  if (:AOrderByPrim = True) then
    sql_stmt = 'COMMENT ON VIEW ' || :relation_name || 
      ...
      ' (created by SP_SDDL_GEN_STD_TABLE_VIEW)''';
  else
    sql_stmt = 'COMMENT ON VIEW ' || :relation_name || 
  ...
  execute
  procedure
    SP_SDDL_GEN_STD_TABLE_VIEW_CMT :ATablename,
      :relation_name;
...
COMMENT ON PROCEDURE SP_SDDL_GEN_STD_TABLE_VIEW IS
'Erstellt eine Standardview zu einem Tabellennamen'^

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_TRIGGER_BI (
  ATablename DN_DB_OBJECT)
returns (
  SUCCESS DN_BOOLEAN)
as
  ...
  begin  
    ...
    new.cre_user = curr_user;
      ...
      sql_stmt = 'COMMENT ON TRIGGER ' || :relation_name || 
        ...
        ' (created by SP_SDDL_GEN_TRIGGER_BI)''';
...
COMMENT ON PROCEDURE SP_SDDL_GEN_TRIGGER_BI IS
'Erstellt den ersten Before-Insert-Trigger BI0 zu einem Tabellennamen um die ID zu erhöhen'^

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_TRIGGER_BU (
  ATablename DN_DB_OBJECT)
returns (
  SUCCESS DN_BOOLEAN)
as
...
begin
  ...
  new.chg_date = current_timestamp;
    ...
    sql_stmt = 'COMMENT ON TRIGGER ' || :relation_name || 
      ...
      ' (created by SP_SDDL_GEN_TRIGGER_BU)''';
  ...
  begin
    ...
    if (Trim(AComment) = '') then
      sql_stmt = 'COMMENT ON SEQUENCE ' || :relation_name || 
        ...
        ' (created by SP_SDDL_GEN_SEQUNECE)''';
    else
      sql_stmt = 'COMMENT ON SEQUENCE ' || :relation_name || 
        ' IS ''' || :AComment || 
        ' (created by SP_SDDL_GEN_SEQUNECE)''';     
...
COMMENT ON PROCEDURE SP_SDDL_GEN_SEQUNECE IS
'Erstellt eine Sequence zu einem Tabellennamen um ein beliebiges Feld zu erhöhen'^

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_CREATE_TBL_CATALOG (
  ATablename DN_DB_OBJECT,
...
BEGIN
  ...
  begin
    ...
    sql_stmt = 'COMMENT ON TABLE ' || :relation_name || ' IS ''Katalog: ' || :relation_name || 
      ' für ' || :cat_comment || ' (created by SP_SDDL_GEN_CREATE_TBL_CATALOG)''';
    execute statement sql_stmt;
    ...
    begin
      ...
      sql_stmt = 'COMMENT ON INDEX ALT_' || :cat_name || ' IS ''(created by SP_SDDL_GEN_CREATE_TBL_CATALOG)''';    
      execute statement sql_stmt;             
    end  
...
COMMENT ON PROCEDURE SP_SDDL_GEN_CREATE_TBL_CATALOG IS
'Erstellt einen Katalog'^

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_CREATE_CATALOG (
...
BEGIN
  ...
  from SP_SDDL_GEN_CREATE_TBL_CATALOG(:relation_name, :domain_name, :cat_comment, True) 
  into :SUCCESS, :cat_name, :table_name; 
 
  /* Standardview anlegen */
  if (SUCCESS = True) then
    select SUCCESS 
    from SP_SDDL_GEN_STD_TABLE_VIEW(:table_name, False) 
    into :SUCCESS;
  
  /* Sequence anlegen */
  if (SUCCESS = True) then
    select SUCCESS 
    from SP_SDDL_GEN_SEQUNECE(:table_name) 
    into :SUCCESS;
   
  /* Trigger anlegen */
  if (SUCCESS = True) then
    select SUCCESS 
    from SP_SDDL_GEN_TRIGGER_BI(:table_name) 
    into :SUCCESS;
  
  if (SUCCESS = True) then
    select SUCCESS 
    from SP_SDDL_GEN_TRIGGER_BU(:table_name) 
    into :SUCCESS;
  
  suspend;
END^

COMMENT ON PROCEDURE SP_SDDL_GEN_CREATE_CATALOG IS
'Legt einen Katalog mit all seinen Datenbankobjekten an'^

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_CREATE_STAMP (
  ATablename DN_DB_OBJECT)
RETURNS (
  SUCCESS DN_BOOLEAN)  
AS
...
COMMENT ON PROCEDURE SP_SDDL_GEN_CREATE_STAMP IS
'Erstellt eine Stempel für eine Tabelle'^

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_FIELD_DESCRIPTION (
  ATablename DN_DB_OBJECT)
RETURNS (
  SUCCESS DN_BOOLEAN)  
AS
...
COMMENT ON PROCEDURE SP_SDDL_GEN_FIELD_DESCRIPTION IS
'Erstellt das Standardfeld: DESCRIPTION'^

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_TABLE_COMMENT (
  ATablename DN_DB_OBJECT,
  AComment DN_COMMENT)
RETURNS (
  SUCCESS DN_BOOLEAN)  
...
COMMENT ON PROCEDURE SP_SDDL_GEN_TABLE_COMMENT IS
'Erstellt einen Kommentar zur Tabelle'^

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_PRIMARY_CONSTRAINT (
  ATablename DN_DB_OBJECT)
RETURNS (
  SUCCESS DN_BOOLEAN)  
AS
...
COMMENT ON PROCEDURE SP_SDDL_GEN_PRIMARY_CONSTRAINT IS
'Erstellt einen Primärschlüssel zur Tabelle'^

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_COMPLETE_TABLE (
  ATablename DN_DB_OBJECT,
...
begin
  ...
  begin
    select SUCCESS
    from SP_SDDL_GEN_FIELD_DESCRIPTION(:table_name)
    into :SUCCESS;
  ...
  begin
    select SUCCESS
    from SP_SDDL_GEN_CREATE_STAMP(:table_name) 
    into :SUCCESS;
  ...
  begin
    select SUCCESS
    from SP_SDDL_GEN_TABLE_COMMENT(:table_name, :AComment)
    into :SUCCESS;
  ...
  begin
    select SUCCESS
    from SP_SDDL_GEN_PRIMARY_CONSTRAINT(:table_name)
    into :SUCCESS;
...
COMMENT ON PROCEDURE SP_SDDL_GEN_COMPLETE_TABLE IS
'Ergänzt ein Grundschema um Standards: Constraints, Sequences, Triggers, Standardviews, etc.'^

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_CHECK_COMMAND (
...
COMMENT ON PROCEDURE SP_SDDL_GEN_CHECK_COMMAND IS
'Prüft ob in einer Feldbeschreibung ein Gencode-Kommand vorhanden ist'^

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_CREATE_UNIQUE_KEY(
  ATablename DN_DB_OBJECT,
...
COMMENT ON PROCEDURE SP_SDDL_GEN_CREATE_UNIQUE_KEY IS
'Erstellt einen Unique-Key'^

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_CREATE_UNIQUE_IDX(
  ATablename DN_DB_OBJECT,
  AFieldname DN_DB_OBJECT,
  AFieldDescription DN_DB_COMMENT)
RETURNS (  
...
begin
  ...
  begin  
    ...
    select
      ...
      TABLENAME
    from
      SP_SDDL_EXTRACT_TABLENAME(:ATablename,
        'TB')
    into
      :determined,
      :table_name;
...
COMMENT ON PROCEDURE SP_SDDL_GEN_CREATE_UNIQUE_IDX IS
'Erstellt einen eindeutigen Einspalten-Index'^

/* wird in SP_SDDL_GEN_CREATE_FOREIGN_KEY für die Implementation benötigt, 
   kann aber erst nach SP_SDDL_GEN_CREATE_FOREIGN_KEY selber implementiert werden*/
CREATE OR ALTER PROCEDURE SP_SDDL_GEN_CREATE_CONSTRAINTS (
  ATablename DN_DB_OBJECT)
RETURNS (
  SUCCESS DN_BOOLEAN,
  LOG_MESSAGE DN_MESSAGE)
...
CREATE OR ALTER PROCEDURE SP_SDDL_GEN_CREATE_FOREIGN_KEY(
  ATablename DN_DB_OBJECT,
  AFieldname DN_DB_OBJECT,
  AFieldDescription DN_DB_COMMENT,
...
begin
  ...
  begin      
    ...
    select DETERMINED, TABLENAME
    from SP_SDDL_EXTRACT_TABLENAME(:AFKTablename, 'TB')
    ...
    begin
      ...
      from SP_SDDL_GEN_CREATE_CONSTRAINTS(:AFKTablename)
...
COMMENT ON PROCEDURE SP_SDDL_GEN_CREATE_FOREIGN_KEY IS
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        from SP_SDDL_GEN_CHECK_COMMAND (:gen_command, :field_description)
        into :found_command_unique_key, :field_description, :fk_table_name, :fk_field_name;
  
        if (:found_command_unique_key = True) then
        begin         
          select SUCCESS, LOG_MESSAGE
          from SP_SDDL_GEN_CREATE_UNIQUE_KEY(:table_name, :field_name, :field_description)
          into :SUCCESS, :LOG_MESSAGE;
        ...
        begin         
          select SUCCESS, LOG_MESSAGE
          from SP_SDDL_GEN_CREATE_UNIQUE_IDX(:table_name, :field_name, :field_description)
          into :SUCCESS, :LOG_MESSAGE;
        ...
        begin         
          select SUCCESS, LOG_MESSAGE
          from SP_SDDL_GEN_CREATE_FOREIGN_KEY(:table_name,
                 :field_name,
                 :field_description,
                 :fk_table_name,
                 :fk_field_name,
...
COMMENT ON PROCEDURE SP_SDDL_GEN_CREATE_CONSTRAINTS IS
'Erstellt für eine Tabelle alle über ein Gencommande angeforderten Constraints'^

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_ALL_CONSTRAINTS
RETURNS (
  SUCCESS DN_BOOLEAN,
  LOG_MESSAGE DN_MESSAGE)
AS
...
begin
  ...
  begin
    for
    select SUCCESS, LOG_MESSAGE
    from SP_SDDL_GEN_CREATE_CONSTRAINTS(:table_name)
    into :SUCCESS, :LOG_MESSAGE
    do
    begin
      Suspend;
...
COMMENT ON PROCEDURE SP_SDDL_GEN_ALL_CONSTRAINTS IS
'Erstellt für alle Tabellen alle über ein Gencommande angeforderten Constraints'^   
    
CREATE OR ALTER PROCEDURE SP_SDDL_GEN_GRANT_VIEW (
...
COMMENT ON PROCEDURE SP_SDDL_GEN_GRANT_VIEW IS
'Erstellt Grants für Views'^   

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_GRANT_SEQ (
  ADBObject DN_DB_OBJECT,  
  AAllRole DN_DB_OBJECT DEFAULT 'SDDL_ALL',
  APublicRole DN_DB_OBJECT DEFAULT 'SDDL_PUBLIC')
AS
...
COMMENT ON PROCEDURE SP_SDDL_GEN_GRANT_SEQ IS
'Erstellt Grants für Sequences'^  

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_GRANT_EXC (
  ADBObject DN_DB_OBJECT,  
  AAllRole DN_DB_OBJECT DEFAULT 'SDDL_ALL',
  APublicRole DN_DB_OBJECT DEFAULT 'SDDL_PUBLIC')
AS
...
COMMENT ON PROCEDURE SP_SDDL_GEN_GRANT_EXC IS
'Erstellt Grants für Exceptions'^ 

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_GRANT_SP (
  ADBObject DN_DB_OBJECT,  
  AAllRole DN_DB_OBJECT DEFAULT 'SDDL_ALL',
  APublicRole DN_DB_OBJECT DEFAULT 'SDDL_PUBLIC',
...
begin 
  ...
  begin
    ...
    begin
      sql_stmt = 'GRANT EXECUTE ON PROCEDURE SP_LOG_HISTORY TO PROCEDURE ' || :ADBObject;
      execute statement sql_stmt;    
    end
    
    if (:AAllowDebug = True) then
    begin
      sql_stmt = 'GRANT EXECUTE ON PROCEDURE SP_LOG_DEBUG TO PROCEDURE ' || :ADBObject;
      execute statement sql_stmt;
    end
  end
end^

COMMENT ON PROCEDURE SP_SDDL_GEN_GRANT_SP IS
'Erstellt Grants für SPs'^   

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_GRANT_SF (
  ADBObject DN_DB_OBJECT,  
  AAllRole DN_DB_OBJECT DEFAULT 'SDDL_ALL',
  APublicRole DN_DB_OBJECT DEFAULT 'SDDL_PUBLIC',
  AAllowLog DN_BOOLEAN = True,
...
begin 
  ...
  begin
    ...
    if (:AAllowLog = True) then
    begin
      sql_stmt = 'GRANT EXECUTE ON PROCEDURE SP_LOG_HISTORY TO FUNCTION ' || :ADBObject;
      execute statement sql_stmt;    
    end
    
    if (:AAllowDebug = True) then
    begin
      sql_stmt = 'GRANT EXECUTE ON PROCEDURE SP_LOG_DEBUG TO FUNCTION ' || :ADBObject;
      execute statement sql_stmt;
    end
  end
end^

COMMENT ON PROCEDURE SP_SDDL_GEN_GRANT_SF IS
'Erstellt Grants für SFs'^

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_GRANT_PKG (
  ADBObject DN_DB_OBJECT,  
  AAllRole DN_DB_OBJECT DEFAULT 'SDDL_ALL',
  APublicRole DN_DB_OBJECT DEFAULT 'SDDL_PUBLIC',
  AAllowLog DN_BOOLEAN = True,
...
CREATE OR ALTER PROCEDURE SP_SDDL_GEN_GRANT_ALL
AS
declare variable relation_name DN_DB_OBJECT;
begin
  for
  ...
  begin
    execute
    procedure
      SP_SDDL_GEN_GRANT_SP (Trim(:relation_name));
  end
  ...
  begin
    execute
    procedure
      SP_SDDL_GEN_GRANT_SF (Trim(:relation_name));
  end
  ...
  begin
    execute
    procedure
      SP_SDDL_GEN_GRANT_PKG (Trim(:relation_name));
  end            
  ...
  begin
    execute
    procedure
      SP_SDDL_GEN_GRANT_VIEW (Trim(:relation_name));
  end      
  ...
  begin
    execute
    procedure
      SP_SDDL_GEN_GRANT_SEQ (Trim(:relation_name));
  ...
  begin
    execute
    procedure
      SP_SDDL_GEN_GRANT_EXC (Trim(:relation_name));
...
COMMENT ON PROCEDURE SP_SDDL_GEN_GRANT_ALL IS
'Erstellt alle Grants für alle Views und SPs'^   

CREATE OR ALTER PROCEDURE SP_SDDL_GEN_GRANT (
  ADBObject DN_DB_OBJECT)
AS
declare variable relation_name DN_DB_OBJECT;
begin
...
COMMENT ON PROCEDURE SP_SDDL_CHECK_STYLEGUIDE_KEYW IS
